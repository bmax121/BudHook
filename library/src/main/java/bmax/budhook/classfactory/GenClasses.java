...
public class GenClasses {
	...
	private static void genField(ClassVisitor cv) {
		cv.visitField(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, GenedClassInfo.paramsFieldName,
				BudCallBack.MethodHookParams.desc, null, null).visitEnd();
		cv.visitField(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, GenedClassInfo.callBackFieldName, BudCallBack.desc, null,
				null).visitEnd();
	}
	...
	private static void genConstructor(ClassVisitor cv, String genClassDesc) {
		...
		MethodVisitor mvInit = cv.visitMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_CONSTRUCTOR, FinalStr.initFuncName,
				FinalStr.voidName + FinalStr.memberDesc + BudCallBack.desc, null, null);
		mvInit.visitCode();
		...
		mvInit.visitTypeInsn(Opcodes.INSN_NEW_INSTANCE, 0, 0, 0, BudCallBack.MethodHookParams.desc);
		mvInit.visitMethodInsn(Opcodes.INSN_INVOKE_DIRECT, BudCallBack.MethodHookParams.desc, FinalStr.initFuncName,
				FinalStr.voidName, new int[] { 0 });
		mvInit.visitFieldInsn(Opcodes.INSN_SPUT_OBJECT, genClassDesc, GenedClassInfo.paramsFieldName,
				BudCallBack.MethodHookParams.desc, 0, 0);
		// BudCallBack0.param.method = arg2;
		mvInit.visitFieldInsn(Opcodes.INSN_SGET_OBJECT, genClassDesc, GenedClassInfo.paramsFieldName,
				BudCallBack.MethodHookParams.desc, 0, 0);
		mvInit.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, BudCallBack.MethodHookParams.desc,
				BudCallBack.MethodHookParams.methodFieldName, FinalStr.memberDesc, 2, 0);
		// BudCallBack0.callBack = arg3;
		mvInit.visitFieldInsn(Opcodes.INSN_SPUT_OBJECT, genClassDesc, GenedClassInfo.callBackFieldName,
				BudCallBack.desc, 3, 0);
	...
	private static void genReplaceMethod(ClassVisitor cv, final GenedClassInfo genedClassInfo) {
		...
		// BudCallBack callBack = getBudCallBack(clazz);2
		...
		mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_STATIC, GenedClassHelper.desc,GenedClassHelper.getMethodHookParamsMethodName,
				BudCallBack.MethodHookParams.desc + FinalStr.classDesc, new int[] { 0 });
		...
		mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_STATIC, GenedClassHelper.desc,GenedClassHelper.getBudCallBackMethodName,
				BudCallBack.desc + FinalStr.classDesc, new int[] { 0 });
		...
		mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, BudCallBack.MethodHookParams.desc,
				BudCallBack.MethodHookParams.argsFieldName, FinalStr.bracket + FinalStr.objectDesc, 0, 1);
		...
		if ( ! isStatic) {
			mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, BudCallBack.MethodHookParams.desc, 
					BudCallBack.MethodHookParams.thisObjectFieldName, FinalStr.objectDesc, regIndex, 1);
			paramIndex ++;
			regIndex ++;
		}
		...
		// call before call
		mvReplace.visitMethodInsn(Opcodes.INSN_INVOKE_VIRTUAL, BudCallBack.desc, BudCallBack.beforeCallMethodName,
		...
		if (!isStatic) {
			mvReplace.visitFieldInsn(Opcodes.INSN_IGET_OBJECT, BudCallBack.MethodHookParams.desc,
					BudCallBack.MethodHookParams.thisObjectFieldName, FinalStr.objectDesc, localRegNo, 1);
			paramIndex++;
			regIndex++;
		}
		...
		if (hookedMethodInfo.hasReturn()) {
			mvReplace.visitFieldInsn(Opcodes.INSN_IGET_OBJECT, BudCallBack.MethodHookParams.desc,
					BudCallBack.MethodHookParams.resultFieldName, FinalStr.objectDesc, 0, 1);
		...
		if (hookedMethodInfo.hasThrowable()) {//
			...
			mvReplace.visitFieldInsn(Opcodes.INSN_IPUT_OBJECT, BudCallBack.MethodHookParams.desc,
					BudCallBack.MethodHookParams.throwableFieldName, FinalStr.throwableDesc, 0, 1);
